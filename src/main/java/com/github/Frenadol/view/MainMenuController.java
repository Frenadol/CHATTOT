...
public class MainMenuController implements Initializable {
    ...
    private void confirmAndAddContact() {
        ...
        if (result.isPresent() && result.get() == ButtonType.OK) {
            // Añadir el contacto al usuario actual
            currentUser.addContactToList(selectedContact);

            // Actualizar y guardar el archivo XML
            List<User> users = XmlReader.getUsersFromXML(filePath);
            for (User user : users) {
            ...
            // Sincronizar contactos en `currentUser` con los actualizados en el XML
            ...
            // Mostrar alerta y actualizar la vista de contactos
        ...
        if (currentUser != null) {
            ...
            for (User user : users) {
                if (user.getName().equals(currentUser.getName())) {
                    // Actualizar la lista de contactos en el objeto `currentUser`
                    currentUser.setContacts(user.getContacts());

                    // Actualizar la lista observable para la interfaz
                    contactList.clear();
